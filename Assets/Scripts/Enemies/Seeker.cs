 ...
 * - Se déplacer dans une direction aléatoire.
 * - Revenir à sa position d'origine.
 * 
 * -------- Détection du joueur : --------
 * Le Seeker détecte automatiquement le joueur s'il est à proximité et qu'il n'est pas caché derrière un obstacle.
 * Quelque que soit la direction du joueur : le seeker a des yeux observant tout son environnement.
 * 
 * -------- Joueur détecté : --------
 * Passage en mode attaque. Si le joueur est visible -> préparation puis lancement de l'attaque à distance.
 * L'attaque à distance génère une orbre qui va légèrement suivre le joueur. Délai entre deux attaques. Si le joueur
 * n'est pas visible, le seeker ne prépare pas son attaque et ne la lance pas.
 * Retour en mode passif -> Lorsque le joueur est trop éloigné ou non visible pendant trop longtemps.
 */
...
public class Seeker : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else //Si le joueur est assez proche pour être attaqué
        {
            /* --------Joueur détecté: --------
            * Passage en mode attaque. Si le joueur est visible->préparation puis lancement de l'attaque à distance.
            * L'attaque à distance génère une orbre qui va légèrement suivre le joueur. Délai entre deux attaques. 
            * Si le joueur n'est pas visible, le seeker ne prépare pas son attaque et ne la lance pas.
            * Retour en mode passif->Lorsque le joueur est trop éloigné ou non visible pendant trop longtemps.
            */
        ...
        )
    {
        ...
        canFreelyMove = false;
        ChangeSeekerAnimation(SeekerAnimState.Thrown);
        //seekerAnimator.SetBool(throwAnimationName, true); //Faire attention au nom renseigné. (Vertical ou Horizontal)
